...
public class SpawnableTypesHelper
{
    ...
    private Dictionary<string, XElement> SpawnableTypes =>
        InitSpawnableTypes() ? _spawnableTypes.OrFail() : throw new ApplicationException("");
    ...
    private bool InitSpawnableTypes()
    {
        ...
        var sptfn = DayzFilesHelper.GetAllSpawnableTypesXmlFileNames(
            _rootDir.OrFail(),
            _mpMissionName.OrFail()
        );
        foreach (var item in sptfn)
        {
            var xd = XDocument.Load(item);
            foreach (
                var xe in xd.Root.OrFail().Nodes().OfType<XElement>().Where(x => x.Name == "type")
            )
            {
    ...
    public IEnumerable<InventoryAttachment> GetAdditionalAttachments(string item)
    {
        if (item.StartsWith("Juggernaut"))
        {
            ...
            {
                SlotName = "",
                Items = new[] { "Juggernaut_Buttpack_Black", "Juggernaut_Pouches_Black", }
                    .Select(
                        className =>
                            new Item
                            {
        ...
        else if (GetSpawnableType(item) is XElement xe)
        {
            ...
            foreach (var attch in xe.Nodes().OfType<XElement>().Where(x => x.Name == "attachments"))
            {
                yield return new InventoryAttachment
                {
                    SlotName = "",
                    Items = (
                        from itm in attch.Nodes().OfType<XElement>().Where(x => x.Name == "item")
                        select new Item
                        {
                            ...
                            Chance =
                                (
                                    itm.Attribute("name")?.Value?.ToUpper()?.Contains("SUPP")
                                        is true
                                    || itm.Attribute("name")?.Value?.ToUpper()?.Contains("SIL")
                                        is true
                                )
                                    ? 0.005
                                    : 1,
