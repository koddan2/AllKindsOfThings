...
namespace N3.CqrsEs.Test
{
    ...
    /// </summary>
    public class TestRouteRegistrar
    {
        ...
        public TestRouteRegistrar(IServiceProvider serviceLocator)
        {
        ...
        public void RegisterInAssemblyOf(IHandlerRegistrar registrar, params Type[] typesFromAssemblyContainingMessages)
        {
            foreach (var typesFromAssemblyContainingMessage in typesFromAssemblyContainingMessages)
            {
                RegisterHandlers(registrar, typesFromAssemblyContainingMessage.GetTypeInfo().Assembly.GetTypes());
            }
        }

        public void RegisterHandlers(params Type[] handlers)
        {
            var registrar = _serviceLocator.GetRequiredService<IHandlerRegistrar>();
            RegisterHandlers(registrar, handlers);
        }
        ...
        public void RegisterHandlers(IHandlerRegistrar registrar, params Type[] handlers)
        {
        ...
        private void InvokeHandler(Type @interface, IHandlerRegistrar registrar, Type executorType)
        {
            ...
            _ = registerExecutorMethod.Invoke(registrar, new object[] { func });
        }
